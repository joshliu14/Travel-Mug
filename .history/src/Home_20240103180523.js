      ...
      for (let j = 0; j < this.state.filterOptions.length; j++) {
        if (
          ...
          cafeObjects[i].methods.indexOf(this.state.filterOptions[j].name) !==
            -1 &&
          validCafes.indexOf(this.state.filterOptions[j].name) === -1
        ) {
          validCafes.push(cafeObjects[i]);
        }
        if (validCafes.indexOf(this.state.filterOptions[j].name) !== -1) {
          validCafes.splice(
            validCafes.indexOf(this.state.filterOptions[j].name),
            1
          );
        }
      }
    }
    const cafes = validCafes.map((cafe, index) => {
      return (
    ...
    return (
      <div>
        ...
        <div className="bottom">
          ...
          <div className="filter-results">{cafes}</div>
        </div>
